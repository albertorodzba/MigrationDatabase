...
public class FilePersonalAdministrativo
{
    ...
    public void storeApplicant(string[,] content)
    {
        ...
        for (int i = 1; i < 21; i++)
        {
            ...
            applicant.Aboutme = content[i, 9].Trim('.');
            ...
            applicant.Children = content[i, 7].Trim('.') == "No Especifica" ? null : content[i, 7].Trim('.');
            ...
            applicant.AdminObservation = content[i, 12].Trim('.');
    ...
    public void storeOtherColumns(Applicant? applicant, string[,] content, int i)
    {
        ...
        /********************DESIRED JOB *****************/
        // |     15 | Puesto administrativo             |
        // |     16 | Auxiliar contable                 |
        // |     17 | Marketing                         |
        // |     18 | Psicólogo/a                       |
        // |     19 | Auxiliar de recursos humanos      |
        // |     20 | Contador                          |
        // |     21 | Administrador de empresas         |
        // |     22 | Administrador de recursos humanos
        ApplicantDesiredJob desiredJob = new ApplicantDesiredJob();

        if (content[i, 3].Contains("Administrativo"))
        {
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 15;
        }

        context.ApplicantDesiredJobs.Add(desiredJob);
        context.SaveChanges();

        if (content[i, 9].Contains("Contable"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 16;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 9].Contains("Marketing"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 17;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 9].Contains("RH"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 19;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 9].Contains("Psicologia"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 18;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 9].Contains("Contabilidad") || content[i, 9].Contains("Contaduria"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 20;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }

        if (content[i, 9].Contains("Administracion De Empresas"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 21;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 9].Contains("Recursos Humanos"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 22;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();
        ...
        if (content[i, 6].Contains("Solter"))
        {
            applicant.FkIdMaritalStatus = 1;
        }
        else
        if (content[i, 6].Contains("Uni")) //union libre
        {
            applicant.FkIdMaritalStatus = 4;
        }
        else
        if (content[i, 6].Contains("Casad"))
        {
            applicant.FkIdMaritalStatus = 2;
        }
        else
        if (content[i, 6].Contains("Separad"))
        {
            applicant.FkIdMaritalStatus = 6;
        }
        else
        if (content[i, 6].Contains("Divorciad"))
        {
            applicant.FkIdMaritalStatus = 5;
        }
        else
        if (content[i, 6].Contains("Viud"))
        {
        ...
        //  3 | Vetada        
        //  4 | No disponible
        if (content[i, 12].Contains("Disponible"))
        {
            applicant.FkIdJobStatus = 2;
        }
        else if (content[i, 12].Contains("Ocup") || content[i, 12].Contains("No Disponible"))
        {
            applicant.FkIdJobStatus = 4;
        }
        else if (content[i, 12].Contains("Betada"))
        {
        ...
        if (content[i, 10].Length > 5 && content[i, 10] != null)
        {
            Console.WriteLine(i + " content[i,10]=> " + content[i, 10] + " has length = " + content[i, 10].Length);
            string phonenumber = content[i, 10].Replace(" ", "");
        ...
        string[] references = content[i, 11].Split("/");
