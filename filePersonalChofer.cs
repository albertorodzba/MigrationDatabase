...
public class FilePersonalChofer
{
    ...
    public void storeOtherColumns(Applicant? applicant, string[,] content, int i)
    {
        ...
        if (content[i, 3].Contains("Chofer"))
        {
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 29;
        }

        context.ApplicantDesiredJobs.Add(desiredJob);
        context.SaveChanges();

        if (content[i, 9].Contains("Particular") || content[i,9].Contains("Mozo"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 30;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 9].Contains("Publico"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 33;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();
        }
        if(content[i,9].Contains("Ejecutiv"))
        {
             desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 31;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();
        }
        if(content[i,9].Contains("Repartidor"))
        {
             desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 32;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();
        }
        ...
        //  3 | Vetada        
        //  4 | No disponible
        if (content[i, 12].Contains("Diponib") || content[i, 13].Contains("En Espera"))
        {
            applicant.FkIdJobStatus = 2;
        }
        else if (content[i, 12].Contains("Ocupad"))
        {
            applicant.FkIdJobStatus = 4;
        }
        else if (content[i, 12].Contains("Betad"))
        {
