...
public class FilePersonalDocente
{
    ...
    public void storeOtherColumns(Applicant? applicant, string[,] content, int i)
    {
        ...
        if (content[i, 3] == "Maestra Natacion")
        {
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 5;
            // print(desiredJob);
        }
        else if (content[i, 3] == "Docente")
        {
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 3;
        }
        else if (content[i, 3].Contains("Niñera"))
        {
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 4;
        }

        context.ApplicantDesiredJobs.Add(desiredJob);
        context.SaveChanges();

        if (content[i, 10].Contains("Educativ"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 6;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 10].Contains("Quimico"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 11;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 10].Contains("Industrial"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 10;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 10].Contains("Puericultura"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 7;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 10].Contains("Preescolar"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 14;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }

        if (content[i, 10].Contains("Primaria"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
            desiredJob.FkIdJob = 12;
            context.ApplicantDesiredJobs.Add(desiredJob);
            context.SaveChanges();

        }
        if (content[i, 10].Contains("Enfermeria"))
        {
            desiredJob = new ApplicantDesiredJob();
            desiredJob.FkIdApplicant = applicant.IdApplicant;
        ...
        if (content[i, 3].Contains("Planta"))
        {
        ...
        {
            Console.WriteLine(i + " content[i,17]=> " + content[i, 11] + " has length = " + content[i, 11].Length);
